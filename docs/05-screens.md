...
─────────────────────────
 左カラム（モバイルは単一列に統合）
   目標演習量
   予想スコア + ドメイン別正答率 + 成績詳細（1パネル）
     → モバイル: サマリーをタップで CombinedDetailModal（スコア推移・ドメイン詳細）
     → デスクトップ: モーダルにせず、同じ白パネル内に区切り線を挟んで
       成績詳細（CombinedDetailModal に `inline` prop）をそのまま続けて表示。
       別カードに分けない＝「サマリーの続き」であることをデザインで示す
 右カラム（デスクトップのみ。モバイルは左カラムに続けて縦積み）
   今日のサービス（日めくり）
