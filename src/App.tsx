...
const API_ENDPOINT = 'https://a0q3656qw4.execute-api.ap-northeast-1.amazonaws.com/dev';
...
function App() {
  ...
  const fetchQuestions = async (type: string) => {
    ...
    try {
      const url = type
        ? `${API_ENDPOINT}/questions?examType=${type}`
  ...
  const fetchDetail = async (id: string) => {
    ...
    try {
      ...
      const data = await res.json();
      setQuestions(prev =>
        prev.map(q => q.questionId === id ? { ...q, ...data } : q)
      );
      setExpandedId(id);
    } catch (err) {
      console.error(err);
  ...
  const selectAll = () => {
    if (selected.size === questions.length) {
      ...
      setSelected(new Set(questions.map(q => q.questionId)));
    }
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 24, fontFamily: "sans-serif" }}>
      <h1 style={{ color: "#232f3e" }}>AWS資格問題一覧</h1>

      <div style={{ marginBottom: 16, display: "flex", gap: 8 }}>
        {["", "CLF", "SAA", "SAP"].map(type => (
          <button
            key={type}
            onClick={() => { setExamType(type); fetchQuestions(type); }}
            style={{
              padding: "6px 16px",
      ...
      <div style={{ marginBottom: 16, display: "flex", gap: 8 }}>
        ...
        <button
          ...
          style={{
            padding: "6px 16px",
            background: selected.size > 0 ? "#232f3e" : "#ccc",
            color: "white",
            border: "none",
            borderRadius: 4,
      ...
      {loading ? (
        ...
        questions.map(q => (
          <div key={q.questionId} style={{
            ...
            marginBottom: 12,
            padding: 16,
            background: selected.has(q.questionId) ? "#fff8ee" : "white",
          }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ marginTop: 8, display: "flex", gap: 8 }}>
                  <button
                    onClick={() => fetchDetail(q.questionId)}
