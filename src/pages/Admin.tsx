...
type Tab = 'questions' | 'reports' | 'tips' | 'import' | 'releases' | 'scan' | 'messages' | 'dailyservice' | 'theme';
...
const THEME_COLOR_LABELS: { key: string; label: string }[] = [
  { key: '--color-primary',       label: 'メインカラー（青）' },
  { key: '--color-primary-hover', label: 'メインカラー・ホバー' },
  { key: '--color-primary-light', label: 'メインカラー・薄' },
  { key: '--color-secondary',     label: 'セカンダリ（ダークネイビー）' },
  { key: '--color-accent',        label: 'アクセント（オレンジ）' },
  { key: '--color-bg-main',       label: '背景色' },
  { key: '--color-text-main',     label: 'テキスト・メイン' },
  { key: '--color-text-sub',      label: 'テキスト・サブ' },
  { key: '--color-border',        label: 'ボーダー' },
];
...
export default function Admin() {
  ...
  const { customColors, applyColors } = useTheme();
      ...
      {tab === 'theme' && (
        <div style={{ maxWidth: 600 }}>
          <p style={{ color: 'var(--color-text-sub)', fontSize: 13, marginTop: 0, marginBottom: 20 }}>
            ライトモードのカラーを変更できます。変更は全ユーザーに即時反映されます。
          </p>
            ...
            {THEME_COLOR_LABELS.map(({ key, label }) => {
              ...
              return (
                <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--color-bg-white)', border: '1px solid var(--color-border)', borderRadius: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--color-text-sub)', fontFamily: 'monospace', background: 'var(--color-bg-main)', padding: '2px 8px', borderRadius: 4 }}>
                    {currentVal}
                  </div>
                  <button
                    onClick={() => {
