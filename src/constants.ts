...
・トラブルシューティング問題を一定割合で出すこと（症状・エラー・接続不可を提示し根本原因/修正を問う）。例: S3イベントでLambdaが起動しない(リソースベースポリシー欠如)、プライベートEC2からS3に届かない(VPCエンドポイント/NAT不在)、AccessDenied(IAM/SCP/KMS)、CloudWatchにメモリが出ない(エージェント未導入)、ALBターゲットがunhealthy(ヘルスチェック/SG)`,
  ML: `・これはAWS認定ではなく、AIF/MLA/AIP が前提とする「AWSサービスに依存しない機械学習の基礎知識」を問う独自カードである。
・**AWSサービス名（SageMaker・Bedrock・Comprehend 等）を問題文・選択肢・解説に一切登場させないこと。** サービス選定ではなく概念そのものを問う。
...
・数式や指標の計算（例: 混同行列から適合率を求める）を含めてよい`,
  DB: `・これはAWS認定ではなく、DEA が前提とする「AWSサービスに依存しないデータベース／SQLの基礎知識」を問う独自カードである。
・**AWSサービス名（RDS・Aurora・DynamoDB・Redshift・Glue 等）を問題文・選択肢・解説に一切登場させないこと。** 汎用的なRDBMS/SQLの概念として問う。
...
・具体的なSQL文を提示して結果や誤りを問う形式を含めてよい（PostgreSQL/標準SQL基準）`,
  NW: `・これはAWS認定ではなく、ANS が前提とする「AWSサービスに依存しないネットワークの実務基礎知識」を問う独自カードである（ANSは5年以上のネットワーク実務経験を推奨）。
・**AWSサービス名（VPC・Route 53・Direct Connect・Transit Gateway 等）を問題文・選択肢・解説に一切登場させないこと。** ベンダー中立なネットワーク理論として問う。
