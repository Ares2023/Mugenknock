...
const CHEAT_DATA: CheatData = {
  CLF: [
    ...
    },
    {
      title: 'コンピューティング',
      items: [
        { name: 'Amazon EC2', desc: '**仮想サーバー**（Elastic Compute Cloud）。OS・ミドルウェアを自由に選択できる。\n購入オプション：\nオンデマンド（使った秒数だけ課金。短期・不規則な用途に最適）・スポット（未使用キャパシティを最大90%割引で利用。中断許容が条件）・リザーブド（1〜3年コミットで最大72%割引。安定した定常ワークロードに最適）・Savings Plans（利用量をコミットする柔軟な割引プラン）', tags: ['オンデマンド', 'スポット', 'リザーブド'] },
  ...
  AIF: [
    {
      ...
      items: [
        ...
        { name: 'ハルシネーション', desc: '**FMが事実と異**なる情報を自信満々に生成してしまう問題（幻覚）。\n対策：\nグラウンディング（回答を引用元のドキュメントに根拠付ける）・RAG（Retrieval-Augmented Generation：検索した文書からのみ回答させる）・温度パラメータ（Temperature）を低くする（出力をより決定論的にする）・ガードレール（誤情報に対してチェックを追加する）', keyword: 'LLM ハルシネーション 幻覚 グラウンディング', tags: ['幻覚', 'グラウンディング', '正確性'] },
  ...
  SAA: [
    {
      ...
      items: [
        ...
        { name: 'Amazon EC2', desc: 'インスタンスタイプ：汎用（M系）・コンピュート最適化（C系）・メモリ最適化（R系）・GPU（P・G系）\n配置グループ（Placement Group）：クラスタ（Cluster：同一ラック内に密集配置・超低レイテンシ・HPC向け）・分散（Spread：各インスタンスを別ラックに分散・可用性向上）・パーティション（Partition：ラックをグループ化して大規模分散DB向け）\nインスタンスストア：EC2に物理的に接続されたNVMe SSD（停止/終了すると**データ消失**）', tags: ['インスタンスタイプ', '配置グループ / Placement Group', 'スポット'] },
        { name: 'AWS Auto Scaling', desc: '**スケーリングポリシー**の種類：\nターゲット追跡（Target Tracking：CPU使用率50%などのメトリクスを目標値に自動調整）・ステップ（Step Scaling：メトリクスの値の幅に応じてスケール量を段階的に設定）・スケジュール（Scheduled：特定の日時に事前にスケール）・起動テンプレート（Launch Template：インスタンスタイプ・AMI・セキュリティグループ等の構成を定義したテンプレート）・ウォームアップ期間（Instance Warmup：新インスタンスが安定するまでメトリクスへの影響を除外する時間）', tags: ['ターゲット追跡 / Target Tracking', '起動テンプレート / Launch Template', 'ウォームアップ'] },
    ...
    },
    {
      title: 'ストレージ',
      items: [
        { name: 'Amazon S3', desc: '**ストレージクラス**（アクセス頻度に応じて選択）：Standard（高頻度アクセス向け・デフォルト）・IA（Infrequent Access：低頻度アクセス。取得時に追加課金）・One Zone-IA（単一AZで低コスト。再作成可能なデータ向け）・Intelligent-Tiering（アクセスパターンを自動学習してクラスを切り替え）・Glacier系（アーカイブ・取得時間とコストがトレードオフ）・CRR（Cross-Region Replication：別リージョンへの自動レプリケーション）・SRR（Same-Region Replication：同リージョン内の別バケットへのレプリケーション）', tags: ['ストレージクラス', 'ライフサイクル', 'レプリケーション'] },
  ...
  DVA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Elastic Beanstalk', desc: '**デプロイポリシー**（デプロイ中のダウンタイムとリスクのトレードオフ）：All-at-once（最速だがデプロイ中にダウンタイムあり）・Rolling（少数ずつ順次更新。容量が一時的に減少）・Rolling with additional batch（余分なインスタンスを追加してから更新。容量を維持）・Immutable（新インスタンス群を並行起動してから切り替え。最も安全）・.ebextensions（リソースや設定をYAMLで追加カスタマイズするファイル・.ebextensions/xxx.config）', tags: ['デプロイポリシー', '.ebextensions', 'Immutable'] },
      ],
    },
    {
      title: 'メッセージング・統合',
  ...
  SOA: [
    {
      ...
      items: [
        ...
        { name: 'AWS CloudTrail', desc: '**AWSリソース**へのAPIコールを記録する監査ログサービス。イベントの種類：管理イベント（AWSリソースの作成・削除・設定変更。デフォルトで有効）・データイベント（S3オブジェクト操作・Lambda関数実行。明示的に有効化が必要）・Insightsイベント（通常と異なるAPI呼び出しパターン・突然の大量呼び出し等を自動検出）', tags: ['管理イベント', 'データイベント', 'Insights'] },
    ...
    },
    {
      title: '自動化・運用',
      items: [
        { name: 'AWS Systems Manager', desc: '主要機能：\nSession Manager（**SSHポート**を開けずにブラウザまたはCLIからEC2にセキュアに接続）・Run Command（複数EC2に対して同時にシェルコマンドやスクリプトを実行）・Patch Manager（OSのセキュリティパッチを自動適用するスケジュール管理）・State Manager（設定の継続的な適用・維持・例：特定のソフトウェアが常にインストール済みであることを保証）・Inventory（EC2のソフトウェア・設定情報を収集）・OpsCenter（運用上の問題・OpsItemを一元管理してRunbookで解決）・Automation（複数ステップの運用タスクを自動化するRunbook・ドキュメントを定義・実行）', tags: ['Session Manager', 'Patch Manager', 'Automation'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Elastic Load Balancing（ELB）', desc: '**ヘルスチェック**設定パラメータ：\n正常しきい値（HealthyThreshold：正常と判断するまでの連続成功回数）・異常しきい値（UnhealthyThreshold：異常と判断するまでの連続失敗回数）・アクセスログ（ELBのアクセスログをS3に保存・デフォルト無効）・クロスゾーン負荷分散（Cross-Zone Load Balancing：複数AZにまたがってトラフィックを均等に分散）・Connection Draining（登録解除の遅延 / Deregistration Delay：登録解除中のターゲットへの既存接続を安全に完了させる猶予時間）', tags: ['ヘルスチェック', 'クロスゾーン / Cross-Zone', 'Connection Draining'] },
        ...
        { name: 'Amazon Route 53（DR構成）', desc: 'DR（**災害対策**）構成の核。ヘルスチェックの種類：エンドポイント監視（HTTP/HTTPS/TCPでエンドポイントの死活を監視）・他ヘルスチェック監視（複数ヘルスチェックのAND/OR評価）・CloudWatchアラーム監視（アラームの状態に連動）\nフェイルオーバールーティングと組み合わせてプライマリ障害時にセカンダリサイトに自動切り替え', tags: ['ヘルスチェック', 'フェイルオーバー', 'DR構成'] },
  ...
  DOP: [
    {
      ...
      items: [
        ...
        { name: 'AWS CodeBuild', desc: '**buildspec.yml** のフェーズ構成：install（ランタイム・依存パッケージをインストール）・pre_build（ビルド前の準備・ECRログイン等）・build（コンパイル・テスト実行・Dockerイメージビルド）・post_build（ECRへのプッシュ・通知）・テストレポート（JUnit・Cucumberなどのテスト結果をCodeBuildに取り込んで可視化）・VPC統合（プライベートリソース・RDS等へのアクセスが必要な場合にVPC内でビルドを実行）', tags: ['buildspec.yml', 'Docker', 'テストレポート'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS CDK（Cloud Development Kit）', desc: '**TypeScript**・Python・Java・C#等のプログラミング言語でCloudFormationテンプレートを生成するIaCフレームワーク。\nConstruct（コンストラクト）の3層：L1（CloudFormationリソースを直接ラップ・低レベル）・L2（AWSサービスを使いやすくした高レベル抽象・セキュアなデフォルト付き）・L3（複数サービスを組み合わせた完全なパターン・例：Static Website Hosting）・CDK（Cloud Development Kit） Pipelines（CDKアプリ自体をCI/CDパイプラインで自動デプロイするライブラリ）', tags: ['CDK', 'Construct', 'CDK Pipelines'], termKeywords: { 'L1': 'L1（CDK Construct）', 'L2': 'L2（CDK Construct）', 'L3': 'L3（CDK Construct）' } },
        { name: 'AWS Systems Manager（DOP観点）', desc: 'Automation Runbook（旧Document）：複数ステップの**運用タスク**をYAMLで定義して自動実行\nState Manager：EC2の設定が常に望ましい状態に保たれることを保証する（設定ドリフトの自動修正）\nRun Command：複数EC2に対して一括でコマンド実行（パッチ確認・ログ収集等）\nParameter Store：階層的な設定値・秘密情報の管理。CDK/CloudFormationとの統合でシームレスに利用', tags: ['Automation Runbook', 'State Manager', '設定継続適用'] },
    ...
    },
    {
      title: '弾力性・セキュリティ',
      items: [
        { name: 'AWS Auto Scaling（DOP観点）', desc: '**ライフサイクルフック**（Lifecycle Hook）：起動時フック（Launching：インスタンス起動後にアプリ設定・エージェントインストールが完了するまで待機）・終了時フック（Terminating：ログ退避・セッション切断などの後処理が完了するまで終了を待機）・予測スケーリング（Predictive Scaling：過去2週間のメトリクスパターンをMLで学習して事前にスケールアウト）・ウォームプール（Warm Pool：停止済みEC2をプールして起動時間を短縮する仕組み）', tags: ['ライフサイクルフック / Lifecycle Hook', '予測スケーリング / Predictive Scaling', 'ウォームプール'] },
        ...
        { name: 'AWS IAM Identity Center（SSO）+ AWS Organizations', desc: '複数の**AWSアカウント**へのシングルサインオン（SSO）をOrganizations全体で一元管理するサービス（旧AWS SSO）。\nアイデンティティソース（認証先の選択）：\nAWS組み込みIDストア（Identity Centerが管理するユーザー/グループ・シンプルな構成）・外部IdP（SAML 2.0：Okta・Azure AD等のIdPと連携。SCIMで自動プロビジョニング）・Active Directory（AWS Managed Microsoft ADまたはAD Connectorで社内ADと同期）・Cognito User Pool連携（CognitoをOIDCアイデンティティソースとしてIdentity Centerに登録することでアプリのユーザーにAWSアカウントアクセスを付与するパターン）・権限セット（Permission Set：アカウントごとに付与するIAMポリシーの組み合わせを定義して一元管理）・Cognitoとの違い（CognitoはB2Cアプリのエンドユーザー認証・ユーザープール/ Identity CenterはAWSアカウントへの社員アクセス管理）', keyword: 'IAM Identity Center AWS SSO Organizations SSO 権限セット Cognito SAML SCIM', tags: ['SSO', '権限セット', 'SAML/SCIM'], seeAlso: ['Organizations（マルチアカウント管理）'] },
  ...
  DEA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Amazon Redshift', desc: '**列指向ストレージ**のDWH（データウェアハウス）。\n分散スタイル（各ノードへのデータ配置方式）：\nKEY（特定カラムの値が同じ行を同じノードに配置・JUSTINでの結合高速化）・ALL（全行を全ノードにコピー・小テーブル向け）・EVEN（ラウンドロビンで均等分散）・AUTO（Redshiftが最適な方式を自動選択）\nソートキー：よく使うWHERE条件カラムに設定してゾーンマップによるスキャン削減\nメンテナンス処理：\nVACUUM（削除マーク行の物理削除＋ソートキー順に再整列・recluster＝再クラスタ化してスキャン効率を回復）・VACUUM REINDEX（インターリーブソートキーの再インデックス・分布が偏ったら実行）・ANALYZE（テーブルの統計情報を更新し、クエリプランナが最適な実行計画を選べるようにする・VACUUMとは別物）\n※ 新しめのRedshiftは自動VACUUM/自動ANALYZE/自動テーブル最適化(ATO)で多くが自動化\nRedshift Spectrum：S3上のデータを直接クエリ（ロード不要）', tags: ['分散スタイル', 'VACUUM/ANALYZE', 'Spectrum'] },
  ...
  MLA: [
    ...
    },
    {
      ...
      items: [
        { name: 'Amazon SageMaker Studio', desc: '**ML開発のため**の統合IDE（開発環境）。Jupyter Notebookを拡張したWebベースの環境で以下を統一UIで利用：Experiments（複数の学習実行の条件・メトリクスを比較管理）・Pipelines（MLパイプラインの定義・実行・可視化）・Model Registry（モデルのバージョン管理・承認）・Clarify（バイアス・説明可能性の分析）', tags: ['IDE', 'Experiments', '統合環境'] },
    ...
    },
    {
      ...
      items: [
        { name: 'Amazon SageMaker Endpoints（推論）', desc: '**推論エンドポイント**の4種類：\nリアルタイムエンドポイント（同期API。低レイテンシが必要な場合）・非同期推論（リクエストをキューに積んでバックグラウンドで処理。大きなペイロードや処理時間が長い推論向け）・サーバーレス推論（トラフィックがゼロの間はコストゼロ。断続的なトラフィック向け）・バッチ変換（S3のデータをバッチ処理。推論エンドポイントの常時起動不要）・マルチモデルエンドポイント（MME：1つのエンドポイントで複数モデルをホスティングしてコスト削減）', tags: ['リアルタイム', '非同期', 'バッチ変換'] },
        ...
        { name: 'Amazon SageMaker Model Monitor', desc: '**デプロイ済みモデル**を継続的に監視する4種類のモニター：データ品質（入力データの統計的特性がベースラインから逸脱していないか・データドリフト）・モデル品質（予測精度が劣化していないか）・バイアスドリフト（特定グループへの偏りが増加していないか）・説明可能性ドリフト（Feature Importanceが変化していないか）', tags: ['データドリフト', 'モデル品質', '継続的監視'] },
      ],
    },
    {
      title: 'MLインフラ・セキュリティ',
  ...
  AIP: [
    {
      ...
      items: [
        { name: 'Amazon Bedrock 基盤モデル', desc: '**単一のAPIで複数**の基盤モデルを呼び出せるサービス。サーバー管理不要。\n利用可能モデル例：\nAmazon Titan（AWSが独自開発したテキスト・エンベディング・画像生成モデル）・Anthropic Claude（テキスト理解・生成・コーディング・分析に優れた大規模言語モデル）・Meta Llama（オープンソースベースの高性能テキスト生成モデル）・Mistral（軽量・高速・多言語対応の高コスパモデル）・Cohere（エンタープライズ向けテキスト分類・エンベディング特化モデル）・Stability AI（画像生成・Stable Diffusion系モデル）', keyword: 'Amazon Bedrock 基盤モデル Claude Titan', tags: ['Claude', 'Titan', 'マルチモデル'] },
        ...
        { name: 'Amazon Bedrock Model Customization', desc: '**FMを自社データ**でカスタマイズする方法：ファインチューニング（Fine-tuning：ラベル付きの入出力ペアで追加学習してタスク特化したモデルを作成）・継続事前学習（Continued Pre-training：ドメイン固有の大量テキストでFMを追加学習して知識を拡充）・Distillation（知識蒸留：大きなモデル・教師の出力を使って小さなモデル・生徒を学習してコンパクト化）', tags: ['ファインチューニング', '継続事前学習', 'Distillation'] },
        { name: 'Amazon Bedrock Model Evaluation', desc: 'モデル評価：自動/人手で比較し**最適なFM**を選定\nモデルを評価・比較してユースケースに最適なモデルを選定するサービス。\n自動評価：精度・堅牢性・毒性等の組み込みメトリクスでモデルをベンチマーク評価\n人間評価：Mechanical Turkや社内チームが出力品質を評価するHuman Evaluationワーカーチームを設定\n評価結果を基に各タスク（要約・分類・Q&A等）に最適なモデルを選定する', tags: ['モデル評価', '自動評価', '比較'] },
      ],
    },
    {
      ...
      items: [
        ...
        { name: 'プロンプトエンジニアリング（AIP）', desc: '**AIP試験**でよく問われるプロンプト技法：System prompt（AIの役割・制約・口調を定義する・「あなたは医療専門家です」等）・Few-shot（入出力例を3〜5件示してフォーマットや判断基準を教える）・Chain-of-Thought（「ステップごとに考えてください」で複雑な推論精度を向上）・XML構造（タグでセクションを区切って指示を明確化・Claudeに特に有効）・ネガティブプロンプト（してはいけないことを明示して誤動作を防止）', keyword: 'プロンプトエンジニアリング Few-shot Chain-of-Thought', tags: ['System prompt', 'Few-shot', 'Chain-of-Thought'] },
        ...
        { name: 'ベクトルDB選択', desc: '**RAG**のベクトル検索バックエンドを選択する基準：Amazon OpenSearch Serverless（ベクトルエンジン：サーバーレスで管理不要。大規模対応）・Aurora PostgreSQL（pgvector拡張：既存のRDSとの統合・SQLでベクトル検索が可能）・Amazon MemoryDB（Redis互換：低レイテンシのインメモリベクトル検索）・Pinecone・Weaviate・Qdrant（外部マネージドベクトルDBサービス。高精度・高機能）', keyword: 'ベクトルデータベース Vector Database pgvector OpenSearch', tags: ['OpenSearch', 'pgvector', 'MemoryDB'] },
      ],
    },
    {
      title: '責任あるAI・ガバナンス',
      items: [
        ...
        { name: 'モニタリングと監査', desc: 'CloudTrail：**Bedrock**のすべてのAPIコール（InvokeModel・RetrieveAndGenerate等）を記録して監査証跡を保持\nCloudWatch メトリクス：\nInvocationCount（モデル呼び出し回数）・InvocationLatency（呼び出しから応答までの時間）・InputTokenCount / OutputTokenCount（トークン使用量）・ModelInvocationThrottledRequests（スロットリングされたリクエスト数）', keyword: 'Bedrock CloudTrail CloudWatch 監視 監査', tags: ['CloudTrail', 'CloudWatch', 'APIログ'] },
      ],
    },
    {
      title: '最適化・運用',
      items: [
        { name: 'コスト最適化', desc: '**料金モデルの選択**：\nオンデマンド（APIコールごとに課金。小規模・不定期な利用に適する）・プロビジョニドスループット（一定スループットを月/6か月/1年コミットで割引購入。大規模・定常的な利用に適する）・プロンプトキャッシュ（Prompt Caching：同じプレフィックスのプロンプト部分をキャッシュして再利用するとトークンコストを削減できる機能）', keyword: 'Bedrock コスト プロビジョニドスループット Prompt Caching', tags: ['プロビジョニドスループット', 'プロンプトキャッシュ', 'コスト'] },
  ...
  ANS: [
    {
      ...
      items: [
        ...
        { name: 'セキュリティグループ vs NACL', desc: 'SG（セキュリティグループ）：ENI（インスタンス）に適用\nステートフル（**戻りパケット**は自動許可・行きを許可すれば戻りは自動）・全ルールを評価してから許可/拒否を決定\nNACL（ネットワークアクセスコントロールリスト）：サブネット境界に適用\nステートレス（行き・戻りを両方とも明示的に許可が必要）・番号の小さいルールから順に評価し最初にマッチを適用', keyword: 'セキュリティグループ NACL ステートフル ステートレス 違い', tags: ['ステートフル', 'ステートレス', '評価順序'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Direct Connect（ANS詳細）', desc: '**Direct** Connect接続の種類：ホスト型（Direct Connectパートナー経由。1/10Gbps等の共有帯域）・専用型（AWSのDirect Connect施設に直接接続。1/10/100Gbpsの専用回線）\nVIF（Virtual Interface：仮想インターフェース）の種別：プライベートVIF（VPCのプライベートリソースへ接続・VGWまたはTGWに接続）・パブリックVIF（S3・DynamoDB等のAWSパブリックエンドポイントへ接続）・トランジットVIF（Transit Gateway経由で複数VPCへ接続）', tags: ['ホスト型', '専用型', 'VIF'] },
        { name: 'AWS Direct Connect 冗長化', desc: '冗長化：**DC複数**やDC+VPNで可用性を高める構成\nDirect Connectの冗長性レベル（AWS推奨）：最大冗長性（2つのDirect Connectロケーション × 2接続 = 4接続。シングルポイント障害なしの最高冗長）・高冗長性（2つのロケーション × 1接続 = 2接続。ロケーション障害に耐性あり）・開発/テスト用（1接続のみ。冗長性なし。本番では非推奨）', tags: ['最大冗長性', '高冗長性', 'HA'] },
        ...
        { name: 'AWS Client VPN', desc: '**リモートワーカー**のPCからAWS VPCへのOpenVPNプロトコルによるVPN接続サービス。\n認証方式：\n相互TLS認証（クライアント証明書とACM証明書で認証）・Active Directory認証（AWS Managed Microsoft ADと連携）・SAML（IdP）認証（OktaやAzure ADなどのSAML 2.0準拠IdPと連携）・スプリットトンネリング（VPC宛のトラフィックのみVPN経由にしてインターネットトラフィックは直接送信・帯域節約）', tags: ['相互TLS', 'AD認証', 'スプリットトンネル'] },
      ],
    },
    {
      title: 'DNS・コンテンツ配信',
  ...
  SCS: [
    ...
    },
    {
      title: 'セキュリティ監視・ログ',
      items: [
        { name: 'AWS Security Hub', desc: 'GuardDuty・Inspector・Macie・Firewall Manager等の検出結果をASFF（Amazon Security Finding Format：**セキュリティ検出結果**の標準形式）で集約・優先順位付けするサービス。\nコンプライアンス基準への自動チェック：\nCIS（Center for Internet Security） AWS Foundations Benchmark（AWSのセキュリティ設定ベースライン）・PCI（Payment Card Industry） DSS（クレジットカード業界のデータセキュリティ基準）・NIST（米国国立標準技術研究所） 800-53（米国政府のセキュリティフレームワーク）', tags: ['ASFF', 'CIS', 'PCI DSS準拠'], seeAlso: ['GuardDuty', 'Inspector', 'Audit Manager'] },
        { name: 'AWS CloudTrail（SCS観点）', desc: '**セキュリティ監査**の中核。イベントの種類：管理イベント（リソースの作成・削除・IAM変更等・デフォルト有効）・データイベント（S3オブジェクト操作・Lambda実行等・明示的に有効化が必要）・Insightsイベント（異常なAPI呼び出しパターンを自動検出）・S3証跡保護（証跡をS3に保存する場合はMFAによる削除防止・KMS暗号化・ログファイル整合性検証・改ざん検出を有効化することが重要）', tags: ['管理イベント', 'データイベント', '整合性検証'] },
        { name: 'Amazon Inspector', desc: '**脆弱性**（セキュリティの弱点）を継続的にスキャンして優先順位付けするサービス。\nスキャン対象：\nEC2インスタンス（エージェント不要でSSMエージェント経由。OSの既知脆弱性を検出）・ECRコンテナイメージ（プッシュ時に自動スキャン）・Lambda関数（コードと依存パッケージの脆弱性をスキャン）・CVE（Common Vulnerabilities and Exposures：既知の脆弱性のIDデータベースと照合してリスクスコア・CVSSで優先順位付け）', tags: ['脆弱性スキャン', 'CVE', 'コンテナ'], seeAlso: ['GuardDuty', 'Security Hub'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS ACM Private CA（プライベート認証局）', desc: 'プライベートPKI（Public Key Infrastructure：公開鍵基盤）を構築して内部サービス・デバイス向けの**TLS証明書**を発行するサービス。\nインターネット向けの公開証明書ではなく、社内マイクロサービス間・VPN・IoTデバイス等の内部TLS通信に使用する。ACMと統合して証明書の自動更新を管理できる。', tags: ['プライベートCA', 'PKI', 'TLS'] },
      ],
    },
    {
      title: 'IAM・アイデンティティ',
      items: [
        ...
        { name: 'AWS Organizations SCP/RCP', desc: 'SCP（サービスコントロールポリシー）：アカウント/OUが持てる**最大権限**の上限を設定\n→ IAMの許可とのAND評価。SCPが許可していないとIAMで許可しても実行できない\nRCP（リソースコントロールポリシー）：S3・KMS・SQS等のリソース側にOrganizations横断で制限を適用\n→ 「このS3バケットにはOrganization外からのアクセスを禁止」といった制御が可能\n2つのアプローチ：\nDenyリスト方式（全てを許可してから禁止事項を明示・デフォルト）・許可リスト方式（全てを禁止してから許可事項を明示・より厳格）', keyword: 'AWS Organizations SCP RCP サービスコントロールポリシー', tags: ['SCP', 'RCP', 'Denyリスト'] },
      ],
    },
    {
      title: 'データ保護',
  ...
  ML: [
    {
      ...
      items: [
        ...
        { name: '過学習と正則化', desc: '過学習（Overfitting）：**訓練データに過剰適合**し、未知データで性能が落ちる問題。\n対策手法：\nL1正則化（Lasso：不要な特徴量の重みをゼロにして特徴量選択の効果）・L2正則化（Ridge：重みを小さく抑えてモデルを単純化）・ドロップアウト（ニューラルネットのニューロンをランダムに無効化して汎化性能を向上）・データ拡張（学習データを水増しして多様性を高める）', keyword: '過学習 Overfitting L1正則化 L2正則化', tags: ['過学習', 'L1/L2正則化', 'ドロップアウト'] },
        ...
        { name: '生成AIの評価指標', desc: '生成AIの品質を測る**代表指標**。\nBLEU（Bilingual Evaluation Understudy：機械翻訳の品質評価。参照訳とのn-gram一致率で測定）・ROUGE（要約品質の評価。参照要約とのn-gram再現率で測定）・BERTScore（BERTの埋め込みを使った意味的類似度評価。表面一致だけでなく意味の近さも考慮）・F1・Precision・Recall（分類タスクの標準指標）・コンバージョン率（AIを活用したマーケティング・レコメンドの最終ビジネス成果を測定するKPI）', keyword: 'BLEU ROUGE BERTScore F1 コンバージョン率 生成AI 評価指標', tags: ['BLEU/ROUGE', 'BERTScore', 'ビジネスKPI'] },
      ],
    },
    {
      title: '学習アルゴリズム',
      items: [
        ...
        { name: '決定木とアンサンブル学習', desc: '決定木 (Decision Tree)：**条件分岐を木構造**で表現。解釈しやすいが単体では過学習しやすい\nアンサンブル学習：複数のモデルを組み合わせて精度を高める手法\nバギング (Bagging)（復元抽出で並列に多数モデルを学習・例：ランダムフォレスト）・ブースティング (Boosting)（前の誤りを次が補正して直列学習・例：勾配ブースティング / XGBoost・LightGBM）', keyword: '決定木 ランダムフォレスト 勾配ブースティング XGBoost バギング ブースティング アンサンブル', tags: ['ランダムフォレスト', '勾配ブースティング / Boosting', 'バギング'] },
