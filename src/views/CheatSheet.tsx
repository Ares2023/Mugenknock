...
const CHEAT_DATA: CheatData = {
  ...
  DEA: [
    ...
    },
    {
      ...
      items: [
        { name: 'AWS KMS（データ暗号化）', desc: 'AWS KMS（データ暗号化）は、**S3・Redshift**・Glue・Athena等のデータサービスと統合して保存データを暗号化するサービス。\nキーポリシー：KMSキーへのアクセスをJSON形式で制御するリソースベースポリシー\nグラント（Grant）：特定の操作（Decrypt等）を特定のAWSプリンシパルに委譲する一時的なアクセス許可の仕組み', tags: ['暗号化', 'キーポリシー', 'グラント'] },
        { name: 'Amazon Macie', desc: 'Amazon Macieは、S3バケット内のPII（Personally Identifiable Information：**個人識別情報**）・認証情報・金融データ等の機密データを機械学習で自動検出・分類するサービス。\nバケットの公開設定ミスも検出する。GDPR・HIPAAなどのコンプライアンス対応に活用される。', tags: ['PII検出', 'S3スキャン', 'データ分類'] },
