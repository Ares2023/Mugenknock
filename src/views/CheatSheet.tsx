  ...
  CLF: [
    {
      ...
      items: [
        ...
        { name: 'AWS Well-Architectedフレームワーク', desc: 'AWS Well-Architectedフレームワークは、**クラウド設計のベストプラクティス**を6本柱で評価するAWS公式のガイドライン。\n① 運用上の優秀性（変化への対応・自動化）\n② セキュリティ（最小権限・暗号化）\n③ 信頼性（障害自動回復・水平スケール）\n④ パフォーマンス効率（適切なリソース選択）\n⑤ コスト最適化（不要リソース排除）\n⑥ 持続可能性（エネルギー効率）', keyword: 'AWS Well-Architected Framework', tags: ['6本柱', '設計', 'ベストプラクティス'] },
        { name: 'AWS CAF（Cloud Adoption Framework）', desc: 'AWS CAF（Cloud Adoption Framework）は、**クラウド移行**を組織全体で成功させる6つの視点のフレームワーク。\nビジネス：ROI・ビジネスケース\n人：スキル・文化変革\nガバナンス：リスク管理・コンプライアンス\nプラットフォーム：アーキテクチャ基盤\nセキュリティ：セキュリティ管理\n運用：運用モデル', keyword: 'Cloud Adoption Framework', tags: ['移行', '6視点', 'フレームワーク'] },
    ...
    },
    {
      title: 'コンピューティング',
      items: [
        { name: 'Amazon EC2', desc: 'Amazon EC2は、OS・ミドルウェアを自由に選べる**仮想サーバー**（Elastic Compute Cloud）。\n購入オプション：\n- オンデマンド：使った秒数だけ課金。短期・不規則な用途に最適\n- スポット：未使用キャパシティを最大90%割引で利用。中断許容が条件\n- リザーブド：1〜3年コミットで最大72%割引。安定した定常ワークロードに最適\n- Savings Plans：利用量をコミットする柔軟な割引プラン', tags: ['オンデマンド', 'スポット', 'リザーブド'] },
        { name: 'AWS Lambda', desc: 'AWS Lambdaは、サーバー管理不要でコードだけ書けばよい**サーバーレス実行環境**（FaaS：Function as a Service）。\nイベント（S3アップロード・API呼び出し・タイマー等）に応じて自動起動し、実行時間のみ課金される。', tags: ['サーバーレス', 'FaaS', 'トリガー'] },
        { name: 'AWS Elastic Beanstalk', desc: 'AWS Elastic Beanstalkは、アプリのコードをアップロードするだけでAmazon EC2・Elastic Load Balancing（ELB）・AWS Auto Scalingを自動構築する**PaaS**サービス。\nインフラを意識せずにアプリを素早くデプロイしたい場合に適している。', tags: ['PaaS', 'デプロイ', '自動設定'] },
        { name: 'Amazon ECS / Amazon EKS', desc: 'Amazon ECS / EKSは、Dockerコンテナを実行・管理する**コンテナオーケストレーション**サービス。\nECS（Elastic Container Service）：AWS独自のコンテナオーケストレーター。シンプルで使いやすい\nEKS（Elastic Kubernetes Service）：Kubernetes（コンテナ管理の業界標準OSS）のマネージドサービス\nどちらもAWS Fargate（サーバーレス起動モード）でAmazon EC2管理を省略できる', tags: ['コンテナ', 'Kubernetes', 'Fargate'] },
        { name: 'Amazon Lightsail', desc: 'Amazon Lightsailは、固定月額でサーバー・SSD・データ転送量が含まれる**シンプルなVPS**（仮想専用サーバー）。料金が予測しやすくWordPress・小規模Web向け。', tags: ['VPS', '固定料金', 'シンプル'] },
      ],
    },
    {
      title: 'ストレージ',
      items: [
        ...
        { name: 'Amazon EFS', desc: 'Amazon EFSは、複数のEC2から同時マウントでき容量が自動スケールする**共有ファイルシステム**（NFS）。', tags: ['NFS', '共有マウント', 'サーバーレス'] },
  ...
  SAA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Amazon EventBridge', desc: 'Amazon EventBridgeは、**AWSサービス**・SaaSアプリ・カスタムアプリのイベントをルールでターゲットに転送するイベントバスサービス。\nイベントパターンマッチングで条件にあうイベントだけ転送。スケジューラとしてcron式での定期実行も可能。\nEventBridge Pipes：以下のパイプラインを簡潔に構築\n① ソース\n② フィルタ\n③ 変換\n④ ターゲット', tags: ['イベントバス', 'ルール', 'スケジューラ'] },
        { name: 'AWS Step Functions', desc: 'AWS Step Functionsは、**Lambda・ECS**・DynamoDB等を組み合わせたワークフローをステートマシン（状態遷移図）として定義・実行・可視化するサービス。\nStandardワークフロー：最大1年・正確に1回実行・実行履歴を保持\nExpressワークフロー：最大5分・高スループット（1秒間に10万実行）', tags: ['ステートマシン', 'ワークフロー', 'サーバーレス'] },
