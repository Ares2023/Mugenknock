...
const CHEAT_DATA: CheatData = {
  CLF: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Lambda', desc: 'サーバーレス実行環境（FaaS: Function as a Service）。サーバー管理不要でコードだけ書けばよい。\nイベント（S3アップロード・API呼び出し・タイマー等）に応じて自動起動し、実行時間のみ課金される。', tags: ['サーバーレス', 'FaaS', 'トリガー'] , seeAlso: ['EventBridge', 'S3', 'SNS', 'SQS']},
    ...
    },
    {
      title: 'ストレージ',
      items: [
        { name: 'S3', desc: '耐久性99.999999999%（11ナイン）のオブジェクトストレージ（ファイルをURLで管理する形式）。\nバケット（≒フォルダのコンテナ）単位でデータを整理し、静的Webサイトのホスティングにも使える。容量無制限で、画像・動画・バックアップ・ログ等の保存に広く使われる。', tags: ['オブジェクト', '11ナイン', '静的ホスティング'] , seeAlso: ['Lambda', 'SNS', 'SQS']},
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'DynamoDB', desc: 'フルマネージドなNoSQLデータベース（SQLを使わないキーバリュー型・ドキュメント型）。\nパーティションキー（+オプションでソートキー）でデータを管理し、一桁ミリ秒の低レイテンシを維持しながら自動でスケールする。', tags: ['NoSQL', '低レイテンシ', 'スケーラブル'] , seeAlso: ['Lambda']},
        { name: 'Aurora', desc: 'AWS独自設計の高性能RDB。MySQL・PostgreSQL互換で既存アプリをそのまま移行できる。\nRDSより最大3倍高速で、6コピーのデータを3つのAZ（アベイラビリティゾーン）に自動分散保存して高耐久性を実現。\nAurora Serverless v2はトラフィックに応じてコンピュートを自動スケールする。', tags: ['MySQL互換', '高性能', 'Aurora Serverless'] , seeAlso: ['Aurora Global Database', 'RDS']},
        ...
        { name: 'Redshift', desc: 'OLAPワークロード（大量データの分析クエリ）向けのデータウェアハウス（DWH）。\n列指向ストレージ（同じ列のデータをまとめて圧縮・格納）により集計クエリを高速処理する。TB〜PBスケールのデータ分析に使用する。', tags: ['DWH', '列指向', '分析'] , seeAlso: ['S3']},
      ],
    },
    {
      title: 'ネットワーキング',
    ...
    },
    {
      title: '管理・監視',
      items: [
        { name: 'CloudWatch', desc: 'AWSリソースとアプリの監視サービス。\nメトリクス: CPU使用率・メモリ・ネットワーク等の数値データをグラフで可視化\nログ: アプリやサービスのログを収集・検索\nアラーム: しきい値を超えたらSNS通知やAutoScalingを自動実行', tags: ['メトリクス', 'ログ', 'アラーム'] , seeAlso: ['EC2', 'SNS']},
        { name: 'CloudTrail', desc: 'AWSアカウントで実行されたすべてのAPIコールを記録する監査ログサービス。\n誰が・どのリソースに・いつ・何をしたかを追跡でき、デフォルトで90日間保持。S3に証跡を保存すれば無期限に保管できる。', tags: ['監査', 'APIログ', 'コンプライアンス'] , seeAlso: ['Lambda', 'S3']},
  ...
  SOA: [
    ...
    },
    {
      title: 'セキュリティ・コスト',
      items: [
        { name: 'GuardDuty', desc: 'CloudTrail・VPCフローログ・DNSクエリログを機械学習と脅威インテリジェンスフィードで分析して脅威を自動検出するサービス。\nEC2のポートスキャン・認証情報の外部への漏洩・S3への不正アクセス等を検出。\nEventBridge → Lambda で自動隔離・通知のパターンが頻出。マルチアカウント（Organizations）にも一括適用可能。', tags: ['脅威検出', '機械学習', '自動応答'], seeAlso: ['Security Hub', 'CloudTrail', 'EC2', 'EventBridge', 'Lambda', 'Organizations', 'S3', 'SNS', 'VPC', 'VPCフローログ'] },
        ...
        { name: 'Compute Optimizer', desc: 'EC2・Lambda・EBS・ECS・Auto Scalingリソースの過去の使用状況を機械学習で分析して適正サイズを推奨するサービス。\nオーバープロビジョニング（無駄なリソース）とアンダープロビジョニング（性能不足）の両方を検出してコスト削減と性能改善を同時に達成できる。', tags: ['適正サイズ', 'EC2推奨', 'コスト最適化'] , seeAlso: ['Auto Scaling', 'CloudWatch', 'EBS', 'EC2', 'Lambda']},
      ],
    },
  ],
  ...
  DOP: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'OpsWorks', desc: 'Chef（Rubyベース）またはPuppet（宣言型）を使ったサーバー設定管理サービス。\nDOPでは複雑な設定管理シナリオや既存のChef/Puppetコードベースを継続利用するケースで登場する。', tags: ['Chef', 'Puppet', '設定管理'] , seeAlso: ['EC2']},
      ],
    },
    {
      title: '監視・インシデント対応',
  ...
  DEA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Step Functions（データパイプライン）', desc: 'Glue・EMR・Lambda・Athena等を組み合わせた複雑なETLパイプラインのオーケストレーション（実行順序・状態管理）サービス。\nDAG（有向非巡回グラフ）として処理フローを定義し、並列実行・条件分岐・エラーリトライを自動的に管理する。', tags: ['オーケストレーション', 'パイプライン', 'ワークフロー'], seeAlso: ['AWS Glue', 'EMR（Elastic MapReduce）', 'Athena', 'Glue', 'Lambda'] },
      ],
    },
    {
      title: 'データストア',
  ...
  MLA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'SageMaker Model Registry', desc: 'モデルのバージョン管理・メタデータ（精度・訓練データ・パラメータ）・承認ワークフローを管理するカタログ。\n承認（Approved）/拒否（Rejected）のステータスを管理し、承認済みモデルのみをCodePipeline・Lambda経由で自動デプロイするパターンが重要。', tags: ['モデル管理', 'バージョン管理', '承認ワークフロー'], seeAlso: ['SageMaker Pipelines', 'SageMaker Studio', 'CodePipeline', 'Lambda'] },
  ...
  SCS: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Inspector', desc: '脆弱性（セキュリティの弱点）を継続的にスキャンして優先順位付けするサービス。\nスキャン対象:\nEC2インスタンス: エージェント不要でSSMエージェント経由。OSの既知脆弱性を検出\nECRコンテナイメージ: プッシュ時に自動スキャン\nLambda関数: コードと依存パッケージの脆弱性をスキャン\nCVE（Common Vulnerabilities and Exposures）: 既知の脆弱性のIDデータベースと照合してリスクスコア（CVSS）で優先順位付け', tags: ['脆弱性スキャン', 'CVE', 'コンテナ'], seeAlso: ['GuardDuty', 'Security Hub', 'EC2', 'Lambda'] },
