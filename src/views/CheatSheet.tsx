...
const CHEAT_DATA: CheatData = {
  CLF: [
    {
      ...
      items: [
        ...
        { name: 'AWS Well-Architectedフレームワーク', desc: 'AWS Well-Architectedフレームワークは、クラウド設計のベストプラクティスを6本柱で評価するAWS公式のガイドライン。\n① 運用上の優秀性（変化への対応・自動化）\n② セキュリティ（最小権限・暗号化）\n③ 信頼性（障害自動回復・水平スケール）\n④ パフォーマンス効率（適切なリソース選択）\n⑤ コスト最適化（不要リソース排除）\n⑥ 持続可能性（エネルギー効率）', keyword: 'AWS Well-Architected Framework', tags: ['6本柱', '設計', 'ベストプラクティス'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Config', desc: 'AWS Configは、リソースの設定変更を継続的に記録し、望ましい状態からの逸脱やコンプライアンス準拠を評価するサービス。\nルール（例：「S3バケットの公開設定を禁止」）を定義して準拠状況を自動チェックし、違反を通知・自動修復できる。', tags: ['設定管理', 'ルール', 'コンプライアンス'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Cost Explorer', desc: 'AWS Cost Explorerは、過去・現在・将来のAWSコストと使用量をグラフで可視化・分析・予測するツール。\nサービス別・リソース別・タグ別・アカウント別にコストをフィルタリングでき、リザーブドインスタンスやSavings Plansの推奨事項も提示してくれる。', tags: ['コスト分析', '可視化', '予測'] },
        { name: 'AWS Budgets', desc: 'AWS Budgetsは、コスト・使用量・RI/Savings Plansがしきい値を超えるとメール等で通知する予算アラートサービス。\n予算の80%・100%に達したらメール通知するよう設定することが多い。', tags: ['予算管理', 'アラート', 'しきい値'] },
  ...
  SAA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Global Accelerator', desc: 'AWS Global Acceleratorは、Anycast IPで世界中のユーザーを最寄りのAWSエッジに誘導し、AWSバックボーン経由で高速転送するサービス。\n2つの固定グローバルIPを提供（ホワイトリスト管理が容易）。非HTTPプロトコル（TCP/UDP）にも対応。CloudFrontはHTTPコンテンツキャッシュ向けで用途が異なる。', tags: ['Anycast', '固定IP', 'バックボーン'] },
  ...
  SOA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Compute Optimizer', desc: 'AWS Compute Optimizerは、EC2・Lambda・EBS等の使用状況を機械学習で分析し、適正なリソースサイズを推奨するサービス。\nオーバープロビジョニング（無駄なリソース）とアンダープロビジョニング（性能不足）の両方を検出してコスト削減と性能改善を同時に達成できる。', tags: ['適正サイズ', 'EC2推奨', 'コスト最適化'] },
      ],
    },
  ],
  ...
  SAP: [
    ...
    },
    {
      title: '移行・モダン化',
      items: [
        { name: 'AWS Application Migration Service（MGN）', desc: 'AWS MGN（Application Migration Service）は、オンプレミスや他クラウドのサーバーをAWSにリフト＆シフト（そのまま移行）するサービス。\nエージェントをソースサーバーにインストールして継続的にAWSへレプリケーション。カットオーバー時のダウンタイムを最小限（分単位）に抑えられる。', tags: ['リフト&シフト', 'エージェント', 'レプリケーション'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Elastic Disaster Recovery（DRS）', desc: 'AWS Elastic Disaster Recovery（DRS）は、ソースサーバーを継続的にAWSへレプリケーションし、PITR（ポイントインタイムリカバリ）で復旧するDRサービス。\nRTO（目標復旧時間）数分・低コスト（平常時はストレージのみ課金）のDRソリューション。フェイルオーバー時にEC2を起動してすぐに業務継続できる。', tags: ['PITR', '低コストDR', 'エージェント'] },
