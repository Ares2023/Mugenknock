  ...
  CLF: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'ECS / EKS', desc: 'コンテナ（Dockerコンテナ）の実行・管理サービス。どちらもFargate（サーバーレス起動モード）でEC2管理を省略できる。\nECS（Elastic Container Service）: AWS独自のコンテナオーケストレーター。シンプルで使いやすい。TaskDefinition（コンテナ定義）→ Service（実行台数管理）→ Clusterの構成。Fargate起動タイプでサーバー管理不要、EC2起動タイプでカスタマイズ可能\nEKS（Elastic Kubernetes Service）: Kubernetes（大規模コンテナ管理のOSS）のマネージドクラスタ。eksctlやkubectlでクラスタ管理', tags: ['コンテナ', 'Kubernetes', 'Fargate', 'TaskDefinition'] , seeAlso: ['EC2']},
  ...
  AIF: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'ハルシネーション', desc: 'FMが事実と異なる情報を自信満々に生成してしまう問題（幻覚）。\n対策:\nグラウンディング: 回答を引用元のドキュメントに根拠付ける\nRAG: 検索した文書からのみ回答させる\n温度パラメータ（Temperature）を低くする: 出力をより決定論的にする\nガードレール: 誤情報に対してチェックを追加する', keyword: 'LLM ハルシネーション 幻覚 グラウンディング', tags: ['幻覚', 'グラウンディング', '正確性'], seeAlso: ['RAG（検索拡張生成）', 'Bedrock Guardrails'] },
  ...
  DVA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'CodeBuild', desc: 'buildspec.yml でビルド手順を定義するサーバーレスのビルドサービス。\nフェーズ: install（ランタイム・依存インストール）→ pre_build（ECRログイン等）→ build（コンパイル・テスト・Dockerイメージビルド）→ post_build（ECRへのプッシュ・通知）\nキャッシュ: ローカルキャッシュ（同一ビルドホスト）またはS3キャッシュで依存関係の再ダウンロードを省略\nテストレポート: JUnit・Cucumberなどのテスト結果をCodeBuildに取り込んで可視化\nVPC統合: プライベートリソース（RDS等）へのアクセスが必要な場合にVPC内でビルドを実行', tags: ['buildspec.yml', 'ビルドフェーズ', 'キャッシュ'] , keyword: 'RDS VPC', seeAlso: ['RDS', 'S3', 'VPC']},        { name: 'CodeDeploy', desc: 'デプロイ先: EC2 / ECS / Lambda / オンプレミスサーバー\nデプロイ種別:\nIn-place: 同じサーバーで旧を停止して新に置き換え（EC2のみ）\nBlue/Green: 新環境を並列起動してトラフィックを切り替え\nデプロイ戦略（EC2）: All-at-once（一斉）→ Rolling（順次）→ Rolling with additional batch → Immutable（新インスタンスで並行）\nライフサイクルフック（EC2/オンプレ）: BeforeInstall → AfterInstall → ApplicationStart → ValidateService\nライフサイクルフック（Lambda/ECS Blue/Green）: BeforeAllowTraffic（スモークテスト）→ AfterAllowTraffic（本番検証）\nBlue/Greenのトラフィック移行:\nCanary: 最初に一部（例: 10%）を流し問題なければ残りを移行（LambdaCanary10Percent5Minutes 等）\nLinear: 一定割合ずつ段階的に移行（LambdaLinear10PercentEvery1Minute 等）\nロールバック: CloudWatchアラームのトリガーで自動ロールバック可能', tags: ['Blue/Green', 'Canary', 'ライフサイクルフック'] , keyword: 'CloudWatch', seeAlso: ['CloudWatch', 'EC2', 'Lambda']},        { name: 'CodePipeline', desc: 'ソースコードの変更を検知して自動でビルド・テスト・デプロイを行うCI/CDパイプライン。\nステージ: Source（CodeCommit/S3/GitHub）→ Build（CodeBuild）→ Test → Deploy（CodeDeploy/ECS/CloudFormation）\nアーティファクト（Artifact）: ステージ間で受け渡すビルド成果物（S3に保存）\n手動承認アクション: 本番デプロイ前に人間の承認を必須にするステップを挿入できる\nクロスアカウントデプロイ: 別AWSアカウントへのデプロイも可能（KMS・S3バケットポリシー設定が必要）\nEventBridgeトリガー: CodeCommitプッシュ・ECRイメージプッシュ等のイベントで自動起動', tags: ['ステージ', '手動承認', 'クロスアカウント'] , keyword: 'EventBridge', seeAlso: ['CloudFormation', 'CodeBuild', 'CodeCommit', 'CodeDeploy', 'EventBridge', 'KMS', 'S3']},        { name: 'SAM（Serverless Application Model）', desc: 'サーバーレスアプリ（Lambda・API Gateway・DynamoDB等）をCloudFormationの拡張構文で簡潔に定義するIaCフレームワーク。\nsam local invoke / sam local start-api: LambdaとAPI Gatewayをローカル環境でエミュレートして開発・テストが可能\nGlobals セクション: 全Lambda関数に共通のタイムアウト・メモリ等を一括設定', tags: ['サーバーレス', 'sam local', 'テンプレート'] , seeAlso: ['API Gateway', 'CloudFormation', 'DynamoDB', 'Lambda']},
  ...
  SOA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'OpsWorks', desc: 'Chef（Rubyベース）またはPuppet（宣言型）を使ったサーバー設定管理サービス。複雑な設定管理シナリオや既存のChef/Puppetコードベースを継続利用するケースで登場する。\nレシピ: ChefでEC2の設定を定義する手順書\nクックブック: レシピのコレクション\nレイヤー: 同じ役割を持つEC2グループ（Webレイヤー・DBレイヤー等）', tags: ['Chef', 'Puppet', '設定管理'] , seeAlso: ['EC2']},
    ...
    },
    {
      title: 'セキュリティ・コスト',
      items: [
        { name: 'GuardDuty', desc: '複数のデータソースを機械学習と脅威インテリジェンスフィードで分析して脅威を自動検出するサービス。\n分析対象: CloudTrail（APIコール）/ VPCフローログ（ネットワーク）/ DNSログ / S3データイベント / EKS監査ログ\n検出例: EC2のポートスキャン・クレデンシャルの外部漏洩・S3への不正アクセス・マイニングマルウェア\nEventBridge → Lambda で自動隔離・SNS通知のパターンが頻出。マルチアカウント（Organizations）にも一括適用可能。', tags: ['脅威検出', '機械学習', '自動応答'], keyword: 'Security Hub SNS', seeAlso: ['Security Hub', 'CloudTrail', 'EC2', 'EventBridge', 'Lambda', 'Organizations', 'S3', 'SNS', 'VPC', 'VPCフローログ'] },        { name: 'Security Hub', desc: 'GuardDuty・Inspector・Macie・Firewall Manager等の検出結果をASFF（Amazon Security Finding Format: セキュリティ検出結果の標準形式）で集約・優先順位付けするサービス。\nコンプライアンス基準への自動チェック:\nCIS AWS Foundations Benchmark: AWSのセキュリティ設定ベースライン\nPCI DSS: クレジットカード業界のデータセキュリティ基準\nNIST 800-53: 米国政府のセキュリティフレームワーク', tags: ['集約', 'ASFF', '準拠状況'], seeAlso: ['GuardDuty', 'Firewall Manager', 'Inspector', 'Macie'] },
  ...
  DEA: [
    {
      ...
      items: [
        ...
        { name: 'DMS（Database Migration Service）', desc: 'ソースDBからターゲットDBへのデータ移行サービス。\n対応: 同種DB間（Oracle→Oracle）と異種DB間（Oracle→Aurora）の両方\nCDC（Change Data Capture）: ソースDBの変更をリアルタイムで継続的にキャプチャしてターゲットに適用。最終カットオーバーのダウンタイムを最小化\nSCT（Schema Conversion Tool）: 異種DB間でSQLスキーマを自動変換するツール\nDMS vs DataSync: DMSはDBレコード移行、DataSyncはファイル/オブジェクト転送', tags: ['DB移行', 'CDC', 'SCT'] , seeAlso: ['Aurora', 'DataSync']},
        ...
        { name: 'DataSync', desc: 'オンプレミスのNFS/SMBファイルサーバー・S3・EFS・FSx・他クラウド間のデータを高速・自動転送するエージェント型サービス。\nエージェント: オンプレ側に仮想アプライアンスをインストールしてAWSと安全に通信\nTLS暗号化・チェックサム検証でデータの整合性を保証。帯域制御とスケジュールも設定可能\nDataSync vs DMS: DataSyncはファイル/オブジェクト転送、DMSはDBレコード移行', tags: ['オンプレ転送', 'エージェント', '自動化'] , seeAlso: ['EFS', 'FSx', 'S3']},
      ],
    },
    {
      title: 'データの変換・処理',
  ...
  SAP: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Migration Hub', desc: 'Application Migration Service・DMS・CloudEndure等のAWS移行ツールの進捗を一元的に追跡・管理するダッシュボード。\nMigration Hub Refactor Spaces: マイクロサービスへのリファクタリング移行を支援するサービス', tags: ['移行追跡', 'ダッシュボード', '一元管理'], seeAlso: ['Application Migration Service（MGN）', 'DMS（Database Migration Service）'] },
      ],
    },
    {
      title: '高度なネットワーキング',
    ...
    <>
      ...
      <PageLayout maxWidth={!isMobile ? 696 : 580} style={!isMobile ? { marginLeft: 0 } : undefined}>
        ...
        <div style={{
          ...
          paddingLeft: 'var(--page-pad-x-mobile)',
          paddingRight: 'var(--page-pad-x-mobile)',
          ...
          <div style={{ display: 'flex', borderBottom: '2px solid var(--color-border)' }}>
            {EXAM_LEVELS.map(({ key, color }) => (
              <button key={key} onClick={() => selectLevel(key as LevelKey)} style={{
                ...
                fontSize: 'var(--font-size-sm2)', whiteSpace: 'nowrap', flexShrink: 0,
            ...
            {currentLevelExams.filter(e => CHEAT_DATA[e]).map(exam => {
              ...
              return (
                <button key={exam} data-exam={exam} onClick={() => selectExam(exam)} style={{
                  ...
                  background: isSelected ? `linear-gradient(145deg, ${levelColor}, ${levelColor}bb)` : 'var(--color-bg-white)',
        ...
        {!isMobile && (
          <div style={{
            ...
            boxShadow: showScrollTop ? 'var(--box-shadow-sm)' : 'none',
          }}>
        ...
        {/* 用語コピーヒント */}
        {!q && (
          <p style={{ fontSize: 'var(--font-size-xs)', color: '#009E9E', marginBottom: 'var(--spacing-sm)', marginTop: 'var(--spacing-sm)' }}>
                ...
                return (
                  <div key={exam}>
                    <div style={{
                      ...
                      marginBottom: 'var(--spacing-sm)', paddingBottom: 6,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-md)' }}>
            {filteredSections.map(section => (
              <div key={section.title}>
                <h2 style={{
                  fontSize: 'var(--font-size-sm)', fontWeight: 700, color: examColor,
                  margin: '0 0 var(--spacing-sm)', paddingBottom: 6,
      ...
      {showScrollTop && (
        <button
          ...
          style={{
            ...
            zIndex: 100, color: 'var(--color-text-main)',
          }}
        >
      ...
      {copiedTerm !== null && (
        <div style={{
          ...
          background: '#009E9E', color: '#fff', padding: '8px 20px',
          borderRadius: 'var(--border-radius-full)',
          fontSize: 'var(--font-size-sm)', fontWeight: 600,
          boxShadow: 'var(--box-shadow-md)', zIndex: 9999,
