...
const CHEAT_DATA: CheatData = {
  CLF: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Elastic Beanstalk', desc: 'アプリのコードをアップロードするだけで、EC2・ELB（ロードバランサー）・Auto Scalingを自動設定するPaaSサービス。\nインフラを意識せずにアプリを素早くデプロイしたい場合に適している。', tags: ['PaaS', 'デプロイ', '自動設定'] },
        ...
        { name: 'Amazon Lightsail', desc: 'シンプルなVPS（仮想専用サーバー）サービス。固定月額料金でサーバー・SSD・データ転送量が含まれるため料金が予測しやすい。WordPress・小規模WebサイトなどEC2より簡単に使いたい場合に最適。', tags: ['VPS', '固定料金', 'シンプル'] },
      ],
    },
    {
      title: 'ストレージ',
      items: [
        { name: 'Amazon S3', desc: '耐久性99.999999999%（11ナイン）のオブジェクトストレージ（ファイルをURLで管理する形式）。\nバケット（≒フォルダのコンテナ）単位でデータを整理し、静的Webサイトのホスティングにも使える。容量無制限で、画像・動画・バックアップ・ログ等の保存に広く使われる。', tags: ['オブジェクト', '11ナイン', '静的ホスティング'] },
        ...
        { name: 'Amazon EFS', desc: '複数のEC2インスタンスから同時マウントできるNFS（Network File System）ファイルシステム。容量は自動でスケールするため事前のサイジング不要。Linux EC2やECS・Lambda・SageMakerなどから利用できる。', tags: ['NFS', '共有マウント', 'サーバーレス'] },
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'Amazon CloudFront', desc: 'グローバルCDN（Content Delivery Network）。世界450以上のエッジロケーションにコンテンツをキャッシュして、ユーザーに最も近いエッジから低レイテンシで配信する。\nオリジン（配信元）にはS3・ALB・EC2・カスタムサーバーを設定できる。', tags: ['CDN', 'エッジ', 'キャッシュ'] },
  ...
  DVA: [
    ...
    },
    {
      title: 'CI/CDとデプロイ',
      items: [
        { name: 'AWS CodeCommit', desc: 'AWSマネージドのプライベートGitリポジトリ。IAMポリシーで細かいブランチ・ファイルレベルのアクセス制御が可能。HTTPS（Git認証情報）またはSSH（公開鍵）で認証。', tags: ['Git', 'IAM認証', 'プライベートリポジトリ'] },
  ...
  DEA: [
    ...
    },
    {
      ...
      items: [
        ...
        { name: 'AWS Lambda（データ処理）', desc: 'Kinesis Data StreamsやDynamoDB Streamsのトリガーで起動してリアルタイムにデータを処理・変換するサーバーレス関数。\n軽量な変換処理やイベント駆動のデータパイプライン（フィルタリング・エンリッチメント・ルーティング）に適している。', tags: ['リアルタイム', 'ストリーム処理', 'イベント駆動'] },
