...
PYEOF
  ...
  bash "$SCRIPT_DIR/sync-drive-recovery.sh" "$target_time" >> "$LOG_DIR/sync_drive.log" 2>&1 &
}
...
PYEOF
...

  {
    ...
    exec_ai() {
      ...
      local _cb
      _cb=$(
        { [ -x /usr/local/bin/claude ] && echo /usr/local/bin/claude; } ||
