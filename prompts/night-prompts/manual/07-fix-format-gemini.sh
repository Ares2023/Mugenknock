...
{"results":[
  ...
  {"questionId":"...","action":"fix","fix":{"questionText":"修正後（変更する場合のみ）","choices":["選択肢Aの完全テキスト","選択肢Bの完全テキスト","選択肢Cの完全テキスト","選択肢Dの完全テキスト"],"explanation":"修正後解説"}}
]}
...
for r in results:
    ...
    if action == 'fix' and fix:
        ...
        qt_val = fix.get('questionText')
        if qt_val and isinstance(qt_val, str) and len(qt_val) >= 10 and qt_val != orig.get('questionText'):
            update_parts.append('questionText = :qt')
            expr_values[':qt'] = {'S': qt_val}
