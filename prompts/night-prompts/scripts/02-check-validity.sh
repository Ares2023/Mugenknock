...
BATCH_SIZE=30
# トークン律速対策: チャンクを大きくして巨大な確認ヘッダー（約6000字）と
# claude 起動ごとのシステムプロンプトの再送回数を減らし、1トークンあたりの確認数を増やす。
# 確認の出力は大半が action=ok（極小）で、fix が混じっても64000トークン上限に余裕がある。
CHUNK_SIZE=10
DEADLINE=""
...
PYEOF
  ...
  while true; do
    ...
    CLAUDE_CODE_MAX_OUTPUT_TOKENS=64000 "$CLAUDE_CMD" -p --tools WebFetch --allowed-tools WebFetch < "$PROMPT_FILE" > "$_STDOUT_F" 2> "$_STDERR_F"
    AI_EXIT=$?
