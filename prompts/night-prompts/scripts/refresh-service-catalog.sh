...
PYEOF
  ...
  timeout -k 30 "${CLAUDE_TIMEOUT:-1800}" "$CLAUDE_CMD" -p --model sonnet --allowed-tools WebFetch < "$PROMPT_FILE" > "$_OUT" 2> "$_ERR"
  AI_EXIT=$?
  ...
  if [ $AI_EXIT -ne 0 ] && echo "$ERRTXT" | grep -q "No such file"; then
    CLAUDE_CMD=$(_find_claude)
    [ -x "${CLAUDE_CMD:-}" ] && { RESULT=$(timeout -k 30 "${CLAUDE_TIMEOUT:-1800}" "$CLAUDE_CMD" -p --model sonnet --allowed-tools WebFetch < "$chunk_file" 2>&1); AI_EXIT=$?; }
  fi
