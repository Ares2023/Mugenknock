...
for q in questions:
    ...
    if exp:
        lines.append(f"explanation(JA): {exp[:200]}")
    if q.get('explanationEn'):
        lines.append(f"explanationEn(現状): {q['explanationEn'][:150]}")
    lines.append("")
print('\n'.join(lines))
PYEOF
    ...
    timeout -k 30 "${CLAUDE_TIMEOUT:-1800}" "$CLAUDE_CMD" -p < "$PROMPT_FILE" > "$_STDOUT_F" 2> "$_STDERR_F"
    AI_EXIT=$?
...
PYEOF
      ...
      _STDOUT_F=$(mktemp /tmp/claude_out_XXXX)
      _STDERR_F=$(mktemp /tmp/claude_err_XXXX)
      timeout -k 30 "${CLAUDE_TIMEOUT:-1800}" "$CLAUDE_CMD" -p < "$TIPS_PROMPT" > "$_STDOUT_F" 2> "$_STDERR_F"
      TIPS_EXIT=$?
...
PYEOF
      ...
      _STDOUT_F=$(mktemp /tmp/claude_out_XXXX)
      _STDERR_F=$(mktemp /tmp/claude_err_XXXX)
      timeout -k 30 "${CLAUDE_TIMEOUT:-1800}" "$CLAUDE_CMD" -p < "$REL_PROMPT" > "$_STDOUT_F" 2> "$_STDERR_F"
      REL_EXIT=$?
